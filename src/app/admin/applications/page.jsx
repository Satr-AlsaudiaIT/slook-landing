...
export default async function AdminApplicationsPage() {
  await requireAdmin()
  const applications = listApplications({ limit: 500 })
    ...
    <div className="mx-auto max-w-4xl">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-white/55">
            Submitted via the public <code className="text-white/80">/apply</code> form.
          </p>
        </div>
      ...
      {applications.length === 0 ? (
        <div className="card-glass rounded-2xl p-10 text-center text-white/55">
          No applications yet. Once someone submits via{' '}
          <code className="text-white/80">/apply</code>, they land here.
        </div>
      ) : (
